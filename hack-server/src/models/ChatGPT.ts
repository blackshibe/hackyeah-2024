...
const ai = {
    ...
    async tags(name, target){
        const tagsCompletion = await openai.chat.completions.create({
            model: "gpt-4o",
            messages: [
                { role: "user", content: `Write tags for company ${name} based on this info: ${target}. Please respond in json` },
            ],
	    });
        const {tags} = JSON.parse(tagsCompletion.choices[0].message.content.substring(8, tagsCompletion.choices[0].message.content.length - 4));
        return tags;
    }
}
export default ai;